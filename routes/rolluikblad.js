...
router.post('/getData', async (req, res) => {
  ...
  try {
    ...
    const _breedte = JSON.parse(req.body.data.breedte);
    const _hoogte = JSON.parse(req.body.data.hoogte);
    const _oppervlak = (Number(_breedte) * Number(_hoogte)) / 1000000;

    const [results, fields] = await connection.execute(
      'SELECT * FROM typelamel WHERE breedte >= ? AND hoogte >= ? AND oppervlak >= ?',
      [_breedte, _hoogte, _oppervlak]
    );
...
router.post('/typeophangveer', async (req, res) => {
  ...
  try {
    ...
    const [results1, fields1] = await connection.execute(`SELECT artikelnummer FROM ophangveren WHERE type = ?`, [_minitradi]);

    if (results1.length > 0) {
      var placeholders = results1.map((item) => `'${item.artikelnummer}'`).join(',');

      const [results2, fields2] = await connection.execute(`SELECT benaming FROM productlijst WHERE artikelnummer IN (${placeholders})`);

      // console.log("results 2de query", results2);
